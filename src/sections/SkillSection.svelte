<script lang="ts">
  ...
  const languageTableData: SkillData[] = [
    ...
    },
    {
      ...
      history: "3年",
      description:
        "Unix系OSを触る上で避けては通れない部分なので一通り学習しました。ファイルの管理に役立っています。以前にはVimを呼び出して使うメモ管理ツール等を開発しました。"
    },
    {
      name: "LaTeX",
      icon: "latex",
  ...
  const frameworkTableData: FrameworkData[] = [
    ...
    },
    {
      ...
      history: "一年",
      description:
        "珍しいRustのゲームライブラリです。特徴であるECSアーキテクチャをとても気に入り、最近はDXライブラリより使っています。Rustの非常に強力な言語機能を使えるのが最高です。"
    }
  ];
